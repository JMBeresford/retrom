use crate::desktop::SaveKind;
use crate::Result;
use crate::SaveManagerExt;
use prost::Message;
use retrom_codegen::retrom::client::saves::SyncEmulatorSaveStatesPayload;
use retrom_codegen::retrom::client::saves::SyncEmulatorSaveStatesResponse;
use retrom_codegen::retrom::client::saves::SyncEmulatorSavesResponse;
...
pub(crate) async fn sync_emulator_saves<R: Runtime>(
    ...
    let result: SyncEmulatorSavesResponse = save_manager
        .check_save_sync_status(emulator_id, SaveKind::Saves)
        .await?
        .into();
    ...
    match result.status() {
        ...
        SaveSyncStatus::CloudNewer => {
            save_manager
                .download_cloud_save_files(emulator_id, SaveKind::Saves)
                .await?;
        }
...
#[command]
#[instrument(skip(app))]
pub(crate) async fn get_emulator_save_states_sync_status<R: Runtime>(
    app: AppHandle<R>,
    emulator_id: i32,
) -> Result<i32> {
    let save_manager = app.save_manager();
...
#[command]
#[instrument(skip(app))]
pub(crate) async fn sync_emulator_save_states<R: Runtime>(
    app: AppHandle<R>,
    payload: Vec<u8>,
) -> Result<Vec<u8>> {
    let payload = SyncEmulatorSaveStatesPayload::decode(payload.as_slice())?;
    let emulator_id = payload.emulator_id;
    let behavior = payload.behavior();
    ...
    match result.status() {
        ...
        SaveSyncStatus::CloudNewer => {
            save_manager
                .download_cloud_save_files(emulator_id, SaveKind::SaveStates)
                .await?;
        }
        SaveSyncStatus::LocalError => match behavior {
            SyncBehavior::ForceCloud => {
