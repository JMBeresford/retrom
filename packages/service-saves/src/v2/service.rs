...
impl EmulatorSavesService for EmulatorSavesServiceHandlers {
    ...
    async fn stat_save_states(
        ...
        let save_states_root = RetromDirs::new().save_states_dir();
        ...
        let save_states_stats: Vec<SaveStatesStat> = files
            ...
            .map(|(emulator_id, files)| {
                ...
                let file_stats = files
                    .into_keys()
                    .filter_map(|fname| PathBuf::from(fname).try_into().ok())
                    .filter_map(|stat: FileStat| stat.relative_to(&emulator_states_dir))
                    .collect();
                ...
                let save_states_directory = emulator_states_dir.to_string_lossy().to_string();
        ...
        Ok(Response::new(StatSaveStatesResponse { save_states_stats }))
    }
    ...
    async fn backup_save_states(
        ...
        Ok(Response::new(BackupSaveStatesResponse::default()))
    }
    ...
    async fn restore_save_states_from_backup(
        ...
        Ok(Response::new(RestoreSaveStatesFromBackupResponse::default()))
    }
}
