export function flatpakManifest(opts: {
  commitSha: string;
  tagValue: string;
  webDistSha: string;
  cargoDepsSha: string;
}) {
  const { commitSha, tagValue, webDistSha, cargoDepsSha } = opts;
...
modules:
  - name: retrom
    ...
    sources:
      ...
      # Static web assets
      - type: archive
        url: https://github.com/JMBeresford/retrom/releases/download/${tagValue}/desktop-frontend-dist.tar.gz
        sha256: "${webDistSha}"
        strip-components: 0

      # Vendored cargo deps
      - type: archive
        url: https://github.com/JMBeresford/retrom/releases/download/${tagValue}/cargo-deps.tar.gz
        sha256: "${cargoDepsSha}"
        strip-components: 0
